...
void FCFS()
{
    ...
    FILE *input_file = fopen("input_file_fcfs.txt", "r");
    FILE *output_file = fopen("output_file_fcfs.txt", "w");
...
{
    struct processsjf *a = (struct process *)p1;
    struct processsjf *b = (struct process *)p2;
...
void SJF()
{
    ...
    input_file = fopen("input_file_sjf.txt", "r");
    output_file = fopen("output_file_sjf.txt", "w");
    ...
    while (completed_processes < num_processes)
    {
        ...
        else
        {
            ...
            completed_processes++;
            execution_order[num_executions++] = processes[shortest_job_index].process_id;
        }
    }
...
{
    FILE *input_file, *output_file;
    input_file = fopen("input_file_rr.txt", "r");
    output_file = fopen("output_file_rr.txt", "w");
        ...
        for (int i = 0; i < num_processes; i++)
        {
            if (processes[i].remaining_time > 0 && processes[i].arrival_time <= current_time)
            {
                ...
                if (processes[i].remaining_time <= quantum_time)
                {
                    ...
                    completed_processes++;
                    execution_order[num_executions++] = processes[i].process_id;
                }
                else
                {
...
{
    struct processps *a = (struct process *)p1;
    struct processps *b = (struct process *)p2;
...
{
    FILE *input_file, *output_file;
    input_file = fopen("input_file_ps.txt", "r");
    output_file = fopen("output_file_ps.txt", "w");
    ...
    while (completed_processes < num_processes)
    {
        ...
        else
        {
            ...
            completed_processes++;
            execution_order[num_executions++] = processes[highest_priority_index].process_id;
        }
    }
...
void SRTF()
{
    ...
    FILE *input_file, *output_file;
    input_file = fopen("input_file_srtf.txt", "r");
    output_file = fopen("output_file_srtf.txt", "w");
    ...
    int arrival_time[n], burst_time[n], waiting_time[n], turnaround_time[n], process_id[n], remaining_time[n];
    for (i = 0; i < n; i++)
    {
    ...
    while (remaining_processes > 0)
    {
        ...
        for (i = 0; i < n; i++)
        {
            if (arrival_time[i] <= current_time && remaining_time[i] < shortest_burst_time && remaining_time[i] > 0)
            {
                shortest_process = i;
...
void LRTF()
{
    ...
    FILE *input_file, *output_file;
    input_file = fopen("input_file_lrtf.txt", "r");
    output_file = fopen("output_file_lrtf.txt", "w");
    fscanf(input_file, "%d", &n);
    ...
    for (int i = 0; i < n; i++)
    {
        fprintf(output_file, "\nP[%d]\t\t%d\t\t%d\t\t%d", i, burst_time[i], waiting_time[i], turnaround_time[i]);
    }
